...
// src/pages/admin/AdminAddPropertyPage.tsx
// ADD mode:  /admin/add-property
// EDIT mode: /admin/add-property?id=prop-010
import { useState, useEffect } from 'react'
import { useNavigate, useSearchParams, Link } from 'react-router-dom'
import { ArrowLeft, Building2, Loader2, CheckCircle2, Plus, X } from 'lucide-react'
...
async function handleSubmit() {
  ...
  try {
    ...
    // only attach images that aren't already saved on this property
    const newImageUrls = form.images.filter(url => !originalImageUrls.includes(url))
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
        ...
        <div className="mt-8 space-y-8">
          ...
          <Section title="Images">
            <FieldGroup>
              <p className="text-xs text-slate-500">Paste an image URL and click Add. The preview appears below.</p>
              <div className="flex gap-2">
                <input
                ...
                />
                <button onClick={addImage} className="flex items-center gap-1 rounded-xl bg-slate-800 px-3 py-2 text-sm text-slate-300 transition hover:bg-slate-700">
                  <Plus size={14} /> Add
                </button>
              </div>
