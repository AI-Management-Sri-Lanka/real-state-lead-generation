...
export default function InquiriesList() {
  ...
  return (
    <DashboardLayout activeNav="Requests">
      <div className="flex flex-1 flex-col bg-slate-50 dark:bg-slate-950 min-h-full">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          {loading ? (
            ...
            <div className="rounded-xl border border-red-200 bg-red-50 p-6 text-center text-red-600 dark:border-red-900/40 dark:bg-red-950/20 dark:text-red-400">
              {error}
            </div>
          ) : inquiries.length === 0 ? (
            ...
            <div className="space-y-4 max-w-5xl mx-auto">
              {inquiries.map((inquiry) => (
                <div key={inquiry.id} className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition hover:border-indigo-100 hover:shadow-md dark:border-slate-800 dark:bg-slate-900 dark:hover:border-indigo-900/40 dark:shadow-none">
                  <div className="flex flex-col lg:flex-row gap-6 justify-between">
                    <div className="space-y-4 flex-1">
                      <div className="flex items-center gap-3">
                        <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-indigo-600 font-semibold dark:bg-indigo-950/40 dark:text-indigo-400">
                      ...
                      <div className="flex flex-wrap gap-4 text-sm">
                        ...
                        {inquiry.propertyTitle && (
                          <div className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
                            <Home size={16} className="text-slate-400 dark:text-slate-500" />
                    ...
                    {inquiry.message && (
                      <div className="lg:w-1/2 rounded-xl bg-slate-50 p-4 border border-slate-200 text-sm text-slate-700 whitespace-pre-wrap dark:bg-slate-950/40 dark:border-slate-800 dark:text-slate-300">
