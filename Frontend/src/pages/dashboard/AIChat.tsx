...
import { Loader2, Plus, Search, Send, Mic, MoreVertical, Trash2, Edit2 } from 'lucide-react'
...
export default function AIChat() {
  ...
  return (
    <DashboardLayout activeNav="AI Chat">
      <div className="min-h-screen bg-page text-slate-100 px-4 py-8 sm:px-6 lg:px-10 xl:px-12">
        <div className="mx-auto grid gap-6 lg:grid-cols-[320px_minmax(0,1fr)]">
          <aside className="rounded-[32px] border border-slate-800/80 bg-slate-950/90 p-5 shadow-panel">
          ...
          <main className="flex min-h-[calc(100vh-10rem)] flex-col overflow-hidden rounded-[32px] border border-slate-800/80 bg-slate-950/90 shadow-panel">
            <div className="flex flex-col gap-3 border-b border-slate-800/90 p-6 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="inline-flex items-center gap-3 rounded-3xl bg-slate-900/90 px-4 py-3 text-sm text-slate-300">
                <span className={`inline-flex h-3 w-3 rounded-full ${isRecording ? 'bg-rose-400 animate-pulse' : 'bg-slate-500'}`} />
