...
export function ChatSessionList({ sessions, onNew, onLoad }: Props) {
  ...
  function openMenu(id: string, anchorEl: HTMLElement) {
    console.log('[ChatSessionList] openMenu', id)
  ...
  function closeMenu() {
    console.log('[ChatSessionList] closeMenu called')
    setMenuOpen(null)
    setMenuPos(null)
  }

  // Belt-and-suspenders: close on Escape, and close on scroll/resize so a
  // portaled, position:fixed menu never ends up floating over the wrong spot.
  useEffect(() => {
    if (!menuOpen) return
  ...
  const activeSession = sessions.find(s => s.id === menuOpen)
  ...
  return (
    ...
    >
                  ...
                  <span
                    ...
                    onClick={e => {
                      ...
                      if (menuOpen === s.id) {
                        ...
                        openMenu(s.id, e.currentTarget as HTMLElement)
                      }
                    }}
                    style={{
      ...
      {/* ── Portaled dropdown ────────────────────────────────────────
          Rendered directly into document.body so it can never be broken
          by an ancestor's transform/filter/overflow, which is what makes
          position:fixed overlays unreliable when nested deep in a layout. */}
      {menuOpen && menuPos && activeSession && createPortal(
        <>
          {/* Full-viewport click-catcher: closes the menu on any outside click */}
          <div
            onClick={() => { console.log('[ChatSessionList] click-catcher hit'); closeMenu() }}
          ...
          >
            {[
              { icon: Pencil, label: 'Rename', action: () => { closeMenu() /* TODO: wire up rename */ } },
              { icon: Trash2, label: 'Delete', action: () => { closeMenu() /* TODO: wire up delete */ }, danger: true },
            ].map(item => {
