...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  // Restore session on mount — reads aimsl_user from localStorage
  useEffect(() => {
  ...
  const googleSignIn = useCallback(async (credential: string) => {
    ...
    setError(null);
    try {
      const userData = await authApi.googleSignIn(credential); // login/signup logic is in authApi
      setUser(userData);                                        // ✅ updates context state
      localStorage.setItem(STORAGE_KEY, JSON.stringify(userData)); // ✅ correct key
    } catch (err) {
      const message = err instanceof Error ? err.message : "Google sign in failed";
      setError(message);
      throw err;
  ...
  return (
    <AuthContext.Provider value={{ user, isAuthenticated: !!user, isLoading, signIn, signUp, googleSignIn, signOut, error }}>
      {children}
    </AuthContext.Provider>
  );
}