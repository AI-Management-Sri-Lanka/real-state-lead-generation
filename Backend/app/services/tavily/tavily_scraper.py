...
class TavilySearchScraper:
    ...
    async def run(self, structured_query: dict) -> list[dict]:
        ...
        location = structured_query.get("location") or ""
        ...
        base_terms = f"{bedroom_str}{ptype} {location}".strip()
...
async def run_tavily_scraper(scraper_input: dict) -> List[ScrapedLead]:
    ...
    query: dict = {}
    if scraper_input.get("hashtags"):
        query["location"] = scraper_input["hashtags"][0]
    if scraper_input.get("keywords"):
        query["property_type"] = scraper_input["keywords"][0]
        query["keywords"] = " ".join(scraper_input["keywords"])
